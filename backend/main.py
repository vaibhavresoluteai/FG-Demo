...
from fastapi import FastAPI, File, UploadFile, HTTPException, WebSocket, Form,Query
from fastapi.middleware.cors import CORSMiddleware
from usecase1 import process_video1,stop_yolo1
from usecase2 import process_video2,stop_yolo2
from usecase3 import process_video3,stop_yolo3
from usecase4 import process_video4,stop_yolo4
...
async def broadcast_images():
    ...
    while True:
        ...
        await asyncio.sleep(0.2)  # Check periodically
...
# Mapping rules to stop functions
stop_rules_mapping = {
    "Crate Count": stop_yolo1,
    "Milk Spillage": stop_yolo2,
    "Milk Wastage": stop_yolo3,
    "Conveyor Belt Crate Count": stop_yolo4,
}

@app.post("/process_stopping/")
async def process_stopping(rule: str = Query(..., alias="process")):
    """
    Dynamically routes to the correct stop function based on the rule.
    """
    if rule not in stop_rules_mapping:
        raise HTTPException(status_code=400, detail="Invalid rule selected.")

    stop_function = stop_rules_mapping[rule]  # Get the corresponding stop function
    stop_function()  # Call the stop function

    return {"message": f"The processing stopped!"}
